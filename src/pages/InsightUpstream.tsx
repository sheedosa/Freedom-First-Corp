...
export const InsightUpstream = () => {
  const content = useContent();
  const { locale } = useLanguage();
  const art = content.insightUpstream;
  const seo = content.seo.insightUpstream;
  ...
  const articleLd = {
    ...
    image: `${SITE_URL}/images/article-4-upstream.jpg`,
    articleSection: art.category,
    inLanguage: locale,
    url: canonicalFor('/insights/upstream-energy-development'),
    mainEntityOfPage: canonicalFor('/insights/upstream-energy-development'),
  ...
  return (
    <div className="flex-grow flex flex-col bg-off-white">
      <Seo
        path="/insights/upstream-energy-development"
        title={seo.title}
        description={seo.description}
        jsonLd={[
          articleLd,
          breadcrumbLd([
            ...
            { name: art.title, path: '/insights/upstream-energy-development' },
          ]),
        ]}
      />
      <ArticleHero
        image="/images/article-4-upstream-banner.jpg"
